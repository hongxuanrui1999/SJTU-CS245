...
def runEasyTL():
    ...
    dist = ['euclidean', 'ma', 'cosine', 'rbf']
    intra_align = ['raw', 'coral']
    for p in pairs:
        print("%s->%s" % (p[0], p[1]))
        ...
        for d in dist:
            for i in intra_align:
                print("dist: %s, intra_align: %s" % (d, i))
                score, _ = EasyTL(X_train, y_train, X_test, y_test, intra_align=i, dist=d)
                with open('EasyTL.txt', 'a') as f:
                    f.write('%s->%s, dist=%s, intra_align=%s, with acc=%f\n' % (p[0], p[1], d, i, score))
        print()
