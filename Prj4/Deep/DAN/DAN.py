...
def test(model):
    ...
    return correct, 100. * correct / tgt_dataset_len

def compareDraw():
    ARacc = np.load('ARacc.npy')
    CRacc = np.load('CRacc.npy')
    PRacc = np.load('PRacc.npy')
    plt.figure()
    plt.plot([200 * i for i in range(1, 51)], ARacc, label='Art-RealWorld')
    plt.plot([200 * i for i in range(1, 51)], CRacc, label='Clipart-RealWorld')
    plt.plot([200 * i for i in range(1, 51)], PRacc, label='Product-RealWorld')
    plt.title('Training Accuracy')
    plt.legend()
    plt.xlabel('Iterations')
    plt.ylabel('Accuracy: %')
    plt.savefig('Acc')
    ARloss = np.load('ARloss.npy', allow_pickle=True)
    CRloss = np.load('CRloss.npy', allow_pickle=True)
    PRloss = np.load('PRloss.npy', allow_pickle=True)
    plt.figure()
    plt.plot(ARloss, label='Art-Realworld')
    ...
    plt.savefig('ARLoss')
    plt.figure()
    plt.plot(CRloss, label='Clipart-Realworld')
    plt.title('Training Loss')
    plt.legend()
    plt.xlabel('Iterations')
    plt.ylabel('Loss')
    plt.savefig('CRLoss')
    plt.figure()
    plt.plot(PRloss, label='Product-Realworld')
    plt.title('Training Loss')
    plt.legend()
    plt.xlabel('Iterations')
    plt.ylabel('Loss')
...
if __name__ == '__main__':
    ...
    train(model)
    _, acc = test(model)
    with open('DAN.txt', 'a') as f:
        f.write('%s->%s with acc=%f' % (src_name, tgt_name, acc))
